# =============================================================
#  데이터로 보는 프로 LoL: 무엇이 강한 선수·강한 팀을 만드는가
#  데이터: Oracle's Elixir (https://oracleselixir.com)  연도별 경기 데이터
#  도구 : R / ggplot2   |  데이터시각화 프로젝트
#
#  실행:  "C:/Program Files/R/R-4.5.3/bin/Rscript.exe" analysis.R
#  결과 :  output/ 폴더에 PNG 차트 + 요약 CSV 생성
# =============================================================
...
# 그래픽 장치(GDI)에 한글 폰트 등록 — 안 하면 한글이 깨짐
...
MAJOR <- c("LCK","LPL","LEC","LCS","LTA","LTA N","LTA S","MSI","WLDs")  # 메이저 리그
POS   <- c("top","jng","mid","bot","sup")
POSK  <- c(top="탑", jng="정글", mid="미드", bot="바텀", sup="서폿")

MAIN_YEARS <- c(2024, 2025)     # 선수 분석 대상 시즌
META_YEARS <- 2014:2025         # 메타 변화 추이 시즌(완료 시즌만; 2026 진행중 제외)
...
save_plot <- function(p, file, w = 9, h = 5.5) {
  tryCatch({
    ggsave(file.path(out_dir, file), p, width = w, height = h, dpi = 150, bg = "white")
    message("  [저장] ", file)
  }, error = function(e) message("  [실패] ", file, " : ", conditionMessage(e)))
}
...
message("선수 분석 로드: ", paste(basename(main_files), collapse = ", "))

raw <- rbindlist(lapply(main_files, fread, showProgress = FALSE), fill = TRUE)
names(raw) <- make.names(names(raw))   # "earned gpm" -> "earned.gpm"
...
message("선수 행: ", nrow(ply),
        "  |  리그: ", paste(sort(unique(ply$league)), collapse = ", "))

# 선수별 시즌 집계(≥20경기) — 산점도/랭킹/레이더 공용
pl_agg <- ply %>%
...
save_plot(p1, "01_position_dpm.png")

## ---- 차트 2) 승/패별 15분 골드 격차 (포지션별) -------------
if ("golddiffat15" %in% names(ply)) {
  ...
  save_plot(p2, "02_golddiff15_winloss.png")
}

## ---- 차트 3) 선수 산점도: 평균 DPM vs 승률 (포지션별) ------
...
save_plot(p3, "03_player_dpm_winrate.png", w = 11, h = 4.5)

## ---- 차트 4) 미드 분당 딜량 Top 10 (막대) -----------------
TARGET <- "mid"
...
save_plot(p4, "04_top10_mid_dpm.png")
...
save_plot(p5, "05_radar_mid.png", w = 7.5, h = 6.5)

## ---- 차트 6) 메타 변화: 시즌별 평균 경기시간 (전 시즌) -----
# 메타 추이는 전 시즌이 필요하므로, 필요한 컬럼만 가볍게 읽어 합칩니다.
...
p6 <- ggplot(meta, aes(year, 평균경기시간)) +
  ...
  labs(title = "메타 변화: 시즌별 평균 경기 시간",
       ...
       x = NULL, y = "평균 경기 시간 (분)")
save_plot(p6, "06_meta_gamelength.png")

## ---- 결과 데이터 저장(제출용) ------------------------------
fwrite(pl_agg, file.path(out_dir, "player_season_summary.csv"), bom = TRUE)  # 엑셀 한글 헤더용 BOM
fwrite(meta,   file.path(out_dir, "meta_by_year.csv"), bom = TRUE)
message("\n완료 — output/ 폴더를 확인하세요.")
