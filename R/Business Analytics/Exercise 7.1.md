...
```
### 13.
BMI. 세계보건기구(World Health Organization)에 의하면, 비만은 전 세계적으로 전염병 수준에 이르렀다.  
(BMI가 적어도 25이면 과체중, 30을 초과하면 비만으로 간주된다).   
백인이면 1이고 그렇지 않으면 0인 더미변수와 함께 대학을 졸업한 30명 남성의 연봉에 대한 데이터.
a. 예측변수들로 BMI와 White를 사용하면서 Salary를 위한 모형을 추정하라.
```
m1 <- lm(Salary ~ BMI + White, data = df)
```
b. 예측변수들로 BMI, White, BMI * White를 사용하면서 Salary를 위한 모형을 다시 추정하라.
```
m2 <- lm(Salary ~ BMI + White + BMI * White, data = df)
```
c. 두 모형 중 어느 모형이 더 적정한가? 설명하라.
   선택된 모형을 사용 BMI가 30인 대학교육을 받은 백인 남성의 연봉을 추정하라.
   백인이 아닌 남성의 연봉을 계산하라.
```
summary(m1) #Adjusted R-squared:  0.7082, p-value: 0.00000002288
summary(m2) #Adjusted R-squared:  0.7344, p-value: 0.0000000292
# 상호작용이 있는 회귀모델이 R-squared가 더 높고 p-value는 비슷해 더 적절하다
predict(m2, newdata = data.frame(BMI = 30, White = 1)) #37.05202
```
300pg
### 14.
Compensation. 한 대기업의 인력관리부서는 성과, 충성도와 평생교육을 촉진시키기 위해 3개 변수,
즉 (1) 사업단위 수익성(Profit, 연간 1000달러), (2) 근무연수(Years), (3) 중간경영자가 대학원학위를 가지고 있는지 여부(Grad: 대학원학위를 가지고 있으면 1, 그렇지 않으면 0)에 기초하여
중간 경영자를 위한 보상(Comp, 연간달러)모형을 개발하기 원한다.
a. 보상을 위한 다음의 모형을 추정하라.
   Comp = b0 + b1Profit + b2Years + b3Grad + b4(Profit * Grad) + b5(Years * Grad) + e
```
m1 <- lm(Comp ~ Profit + Years + Grad + Profit * Grad + Years * Grad, data=df)
```
b. 5%의 유의수준에서 회귀모형은 유의한가?
```
summary(m1) #p-value: 0.00000000004271 유의하다
```
c. 5%의 유의수준에서 어떤 예측변수들과 상호작용변수들이 유의한가?
```

Profit         10.3154     1.8308   5.634 0.00000389 ***
  Years        1227.6866   277.1757   4.429   0.000116 ***
  Grad        36655.1363 12297.6156   2.981   0.005658 ** 
  Profit:Grad    -0.5227     2.4584  -0.213   0.833077    
Years:Grad   -193.1612   343.4143  -0.562   0.577973 
Years, Grad가 유의하다
```
d. 회귀모형을 사용하여 대학원학위를 가지고 있고 작년에 사업단위 수익이 4,800,000달러였으며 근무연수가 15년인 경영자를 위한 보상을 결정하라
```
predict(m1, newdata = data.frame(Profit = 4800000, Years = 15, Grad = 1, Comp = 1)) #47060009
```
### 15.
